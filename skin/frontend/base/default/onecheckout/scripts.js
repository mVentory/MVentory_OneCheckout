...
OneCheckout.prototype = {
	...
	setLoading: function(flag) {
		...
		var areas = this.updateAreas;
		
		for (i = 0; i < areas.length; i++) {
			var $area = $("checkout-" + areas[i] + "-load");
    ...
    evaluateResponse: function(transport){
        if (transport && transport.responseText) {		
            ...
            if (response.updates)
              for (var param in response.updates)
                if (response.updates.hasOwnProperty(param)) {
